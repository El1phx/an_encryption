...
void transformBlock(std::vector<unsigned char>& block, const std::vector<unsigned char>& roundKey) {
    const int blockSize = block.size(); // 128字节
    ...
    for (int row = 0; row < 16; ++row) { // 128字节视为16行x8列
        ...
        if (row % 2 == 0) {
            std::rotate(block.begin() + rowStart,
...
// 主加密/解密函数
void selfInverseCipher(std::vector<std::vector<unsigned char>>& blocks,
    const std::vector<unsigned char>& masterKey) {
    const int blockSize = 128; // 1024位 = 128字节
